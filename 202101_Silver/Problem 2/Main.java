...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // C[i]: color of segment i
        int[] C = new int[n];
        for(int i=0; i < n; i++) C[i] = tmp[i] - 'A';

        // M[i]: last occurred pos of color i
        int[] M = new int[26];
        Arrays.fill(M, -1);
        M[C[0]] = 0;

        // L[i]: strokes needed to paint from 0 to i
        int[] L = new int[n];
        L[0] = 1;
        for(int i=1; i < n; i++) {
            L[i] = L[i - 1];
        ...
        // R[i]: strokes needed to paint from i to n - 1
        int[] R = new int[n];
        R[n - 1] = 1;
        for(int i=n - 2; i >= 0; i--) {
            R[i] = R[i + 1];
    ...
    // Test if a potential earlier stroke of color c on the left is blocked off by a lighter color
    public static boolean tl(int[] M, int c) {
        for(int i=0; i < c; i++) if(M[i] > M[c]) return true;
        return false;
    }

    // Test if a potential earlier stroke of color c on the right is blocked off by a lighter color
