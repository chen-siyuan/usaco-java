...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        // P[i]: current num in pos i
        int[] P = new int[n];
        for(int i=0; i < n; i++) P[i] = i;

        // H[i]: history of pos of num i
        ...
        for(int i=0; i < n; i++) union(par, i, P[i]);
        for(int i=0; i < n; i++) find(par, i);

        for(int i=0; i < n; i++) H[par[i]].addAll(H[i]);

        for(int i=0; i < n; i++) System.out.println(H[par[i]].size());
